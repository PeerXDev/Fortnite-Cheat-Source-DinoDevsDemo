...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, std::vector<uint8_t>data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, bool mdlMode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {

	const PIMAGE_NT_HEADERS64 nt_headers = portable_executable::GetNtHeaders(data.data());
	...
	do {
		...
		memcpy(local_image_base, data.data(), nt_headers->OptionalHeader.SizeOfHeaders);
		...
		for (auto i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i) {
			...
			memcpy(local_section, reinterpret_cast<void*>(reinterpret_cast<uint64_t>(data.data()) + current_image_section[i].PointerToRawData), current_image_section[i].SizeOfRawData);
		}
